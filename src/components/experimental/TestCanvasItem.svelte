<script>
	...
	function HandleX(Direction, FrameX, FrameW) {
		// extra arg.s for reactivity
		switch (Direction) {
			case 'nw':
			case 'w':
			case 'sw':
				return FrameX - HandleW
			case 'n':
			case 's':
				return FrameX + Math.round((FrameW - HandleW) / 2)
			case 'ne':
			case 'e':
			case 'se':
				return FrameX + FrameW
		}
	}

	function HandleY(Direction, FrameY, FrameH) {
		// extra arg.s for reactivity
		switch (Direction) {
			case 'nw':
			case 'n':
			case 'ne':
				return FrameY - HandleH
			case 'e':
			case 'w':
				return FrameY + Math.round((FrameH - HandleH) / 2)
			case 'sw':
			case 's':
			case 'se':
	...
	function onHandleDragMove(Direction) {
		return (Event) => {
			...
			switch (Direction) {
				...
				case 'w':
				case 'sw':
					HandleX = Math.max(FrameR - maxFrameW, Math.min(FrameR - minFrameW, HandleX))
					break
				case 'ne':
				case 'e':
				case 'se':
					HandleX = Math.max(FrameX + minFrameW, Math.min(FrameX + maxFrameW, HandleX))
			}
			...
			switch (Direction) {
				...
				case 'n':
				case 'ne':
					HandleY = Math.max(FrameB - maxFrameH, Math.min(FrameB - minFrameH, HandleY))
					break
				case 'sw':
				case 's':
				case 'se':
					HandleY = Math.max(FrameY + minFrameH, Math.min(FrameY + maxFrameH, HandleY))
			}
			...
			switch (Direction) {
				case 'nw':
					DeltaX = HandleX + HandleW - FrameX
					FrameX += DeltaX
					FrameW -= DeltaX
					DeltaY = HandleY + HandleH - FrameY
					FrameY += DeltaY
					FrameH -= DeltaY
					break
				case 'n':
...
>
	<div
		class="draggable SelectionFrame"
		style="
	...
	<div
		class="draggable ResizeHandle"
		style="
      left:{HandleX('nw', FrameX, FrameW)}px; top:{HandleY(
			'nw',
			FrameY,
			FrameH
		)}px; cursor:nwse-resize
    "
	...
	<div
		class="draggable ResizeHandle"
		style="
      left:{HandleX('ne', FrameX, FrameW)}px; top:{HandleY(
			'ne',
			FrameY,
			FrameH
		)}px; cursor:nesw-resize
    "
		use:draggable="{{ helper: 'clone', revert: true }}"
	...
	<div
		class="draggable ResizeHandle"
		style="
      left:{HandleX('se', FrameX, FrameW)}px; top:{HandleY(
			'se',
			FrameY,
			FrameH
		)}px; cursor:nwse-resize
	...
	<div
		class="draggable ResizeHandle"
		style="
      left:{HandleX('sw', FrameX, FrameW)}px; top:{HandleY(
			'sw',
			FrameY,
			FrameH
		)}px; cursor:nesw-resize
